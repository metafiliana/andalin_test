...
module.exports = {
        ...
        movieModel.find({}, function(err, movies){
            if (err){
                ...
                for (let movie of movies) {
                    moviesList.push({id: movie._id, name: movie.name, released_on: movie.released_on, description: movie.description});
                }
                
                res.json({status:"success", message: "", data:{movies: moviesList}});
            }
        });
    },
    ...
    create: function(req, res, next) {
        movieModel.create({ name: req.body.name, released_on: req.body.released_on, description: req.body.description }, function (err, result) {
            if (err) 
                next(err);
            else
